...
type PredictionRecord = {
  ...
  wti: WtiMarket & { weight: number; correlation: number };
...
function getCorrelation(btcPoints: PricePoint[], wtiPoints: PricePoint[]) {
  const length = Math.min(btcPoints.length, wtiPoints.length, 30);

  if (length < 4) {
    return 0;
  }

  const btcReturns = btcPoints.slice(-length).slice(1).map((point, index, items) => {
    const previous = index === 0 ? btcPoints.slice(-length)[0] : items[index - 1];
    return (point.price - previous.price) / previous.price;
  });
  const wtiReturns = wtiPoints.slice(-length).slice(1).map((point, index, items) => {
    const previous = index === 0 ? wtiPoints.slice(-length)[0] : items[index - 1];
    return (point.price - previous.price) / previous.price;
  });
  const btcMean = btcReturns.reduce((sum, value) => sum + value, 0) / btcReturns.length;
  const wtiMean = wtiReturns.reduce((sum, value) => sum + value, 0) / wtiReturns.length;
  const covariance = btcReturns.reduce((sum, value, index) => sum + (value - btcMean) * (wtiReturns[index] - wtiMean), 0);
  const btcVariance = btcReturns.reduce((sum, value) => sum + (value - btcMean) ** 2, 0);
  const wtiVariance = wtiReturns.reduce((sum, value) => sum + (value - wtiMean) ** 2, 0);

  if (!btcVariance || !wtiVariance) {
    return 0;
  }

  return covariance / Math.sqrt(btcVariance * wtiVariance);
}
...
export default function Btc15MinuteAnalysisPage() {
  ...
  const [wti, setWti] = useState<WtiMarket>({
    price: 0,
    points: [],
    oneMinuteReturn: 0,
    fiveMinuteReturn: 0,
    fifteenMinuteReturn: 0,
    percentChange: 0,
    volatility: 0
  });
  const [wtiFactorMultiplier, setWtiFactorMultiplier] = useState("1");
  ...
  useEffect(() => {
    ...
    async function fetchWti() {
      const response = await fetch("/api/markets-wti", { cache: "no-store" });

      if (!response.ok) {
        return;
      }

      const data = (await response.json()) as WtiMarket;
    ...
    fetchWti();
    const interval = window.setInterval(fetchWti, 15_000);
  ...
  const indicators = useMemo(() => {
    ...
    const correlation = getCorrelation(pricePoints, wti.points);
    const adaptiveWtiWeight = stats.wtiWeight || 0.08;
    const manualWtiMultiplier = Number(wtiFactorMultiplier) || 1;
    const wtiWeight = adaptiveWtiWeight * manualWtiMultiplier;
    const wtiFactor = wti.fifteenMinuteReturn * wtiWeight * (correlation || 0.35);
    const impliedYesProbability = clamp(Number(yesPrice) / (Number(yesPrice) + Number(noPrice)), 0, 1) || 0.5;
    const score = returnFromStart * 9 + momentum * 22 + volumeImbalance * 0.15 + bookImbalance * 0.12 + wtiFactor - volatility * 8;
  ...
  }, [askDepth, bidDepth, currentPrice, initialPrice, noPrice, pricePoints, stats.wtiWeight, trades, wti, wtiFactorMultiplier, yesPrice]);
  ...
  async function saveCurrentPrediction() {
    const response = await fetch("/api/btc-15m-predictions", {
      ...
      body: JSON.stringify({
        ...
        indicators,
        wti: { ...wti, weight: indicators.wtiWeight, correlation: indicators.correlation },
    ...
    <main className="min-h-screen bg-coffee-black px-5 py-10 text-coffee-cream md:px-8">
      <section className="mx-auto max-w-7xl">
        ...
        <h1 className="mt-4 max-w-5xl font-serif text-4xl leading-tight md:text-6xl">
          Sistema multi-mercado para predicciones BTC de 15 minutos.
        </h1>
        <p className="mt-5 max-w-3xl text-lg leading-8 text-coffee-cream/75">
          Este dashboard analiza Coinbase BTC-USD y WTI Crude Oil en tiempo real para registrar predicciones y hacer backtesting. No coloca órdenes, apuestas ni operaciones automáticamente.
        </p>
      ...
      <section className="mx-auto mt-10 grid max-w-7xl gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-4 border border-coffee-gold/30 bg-coffee-espresso p-5 shadow-luxury">
          ...
          <label className="block text-sm uppercase tracking-[0.16em] text-coffee-gold">Factor WTI configurable</label>
          <input value={wtiFactorMultiplier} onChange={(event) => setWtiFactorMultiplier(event.target.value)} className="w-full bg-coffee-black px-4 py-3 text-coffee-cream" placeholder="1 = peso automático" />
          <p className="text-xs leading-6 text-coffee-cream/65">Peso automático actual: {formatPercent(indicators.wtiWeight)} · {stats.wtiImprovesAccuracy ? "WTI mejora accuracy histórica" : "WTI reducido hasta tener mejora histórica"}</p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {[
            ...
            ["WTI Crude Oil", formatUsd(wti.price)],
            ["Correlación", getCorrelationLabel(indicators.correlation)]
          ].map(([label, value]) => (
      ...
      <section className="mx-auto mt-8 grid max-w-7xl gap-4 md:grid-cols-3">
        {[
          ...
          ["WTI 1m", formatPercent(wti.oneMinuteReturn)],
          ["WTI 5m", formatPercent(wti.fiveMinuteReturn)],
          ["WTI 15m", formatPercent(wti.fifteenMinuteReturn)],
          ["WTI cambio %", formatPercent(wti.percentChange)],
          ["WTI volatilidad", wti.volatility.toFixed(5)],
          ["Factor WTI", indicators.wtiFactor.toFixed(5)]
        ].map(([label, value]) => (
      ...
      <section className="mx-auto mt-10 grid max-w-7xl gap-6 lg:grid-cols-2">
        <article className="border border-coffee-gold/30 bg-coffee-espresso p-5">
          <p className="text-xs uppercase tracking-[0.18em] text-coffee-gold">Correlation panel</p>
          <h2 className="mt-2 font-serif text-3xl">BTC ↔ WTI Crude Oil</h2>
          <p className="mt-4 text-2xl text-coffee-parchment">{getCorrelationLabel(indicators.correlation)}</p>
          <p className="mt-2 text-coffee-cream/70">Strength: {formatPercent(Math.abs(indicators.correlation))}</p>
        </article>
        <article className="border border-coffee-gold/30 bg-coffee-espresso p-5">
          <p className="text-xs uppercase tracking-[0.18em] text-coffee-gold">AI summary</p>
          <div className="mt-4 grid gap-2 text-lg">
            <p>{indicators.fifteenMinuteReturn >= 0 ? "BTC Bullish" : "BTC Bearish"}</p>
            <p>{wti.fifteenMinuteReturn >= 0 ? "Oil Bullish" : "Oil Bearish"}</p>
            <p>Correlation Strength: {formatPercent(Math.abs(indicators.correlation))}</p>
            <p>Overall Confidence: {formatPercent(Math.abs(indicators.modelEdge) + Math.abs(indicators.correlation) * 0.2)}</p>
          </div>
        </article>
      </section>
          ...
          <div className="max-h-[440px] overflow-auto">
            {predictions.map((prediction) => (
              <article key={prediction.id} className="border-b border-coffee-gold/15 p-5 text-sm leading-7">
                ...
                <p>WTI: {formatUsd(prediction.wti?.price || 0)} · Corr: {formatPercent(prediction.wti?.correlation || 0)} · Peso WTI: {formatPercent(prediction.wti?.weight || 0)}</p>
