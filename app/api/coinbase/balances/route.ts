...
function getEd25519Seed(privateKey: string) {
  const decodedKey = Buffer.from(privateKey.replace(/\\n/g, "").replace(/\s/g, ""), "base64");

  if (decodedKey.length !== 64) {
    throw new Error("COINBASE_ADVANCED_PRIVATE_KEY must be a base64 Ed25519 key secret.");
  }

  return decodedKey.subarray(0, 32);
}

function createEd25519PrivateKey(privateKey: string) {
  const seed = getEd25519Seed(privateKey);
...
function createCoinbaseJwt(method: string, requestPath: string) {
  ...
  if (!keyName || !privateKey) {
    throw new Error(
      "Coinbase Advanced API is not configured. Add COINBASE_ADVANCED_API_KEY and COINBASE_ADVANCED_PRIVATE_KEY for an Ed25519 key."
    );
  }
  ...
  const keyObject = createEd25519PrivateKey(privateKey);
  const signature = sign(null, Buffer.from(tokenBody), keyObject);
